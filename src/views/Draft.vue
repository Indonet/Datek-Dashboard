<template>
  <v-container>
    <v-card>
      <v-card-text>
          ...
          <v-col cols="12">
            <v-card :title="memo.code" class="text-white bg-animate" flat>
              <template v-slot:prepend>
                ...
                >
                  <div
                    class="text-capitalize text-caption px-5 py-2 mt-8"
              ...
              <template v-slot:append>
                ...
                >
                  <span v-if="!$vuetify.display.sm">Delete</span>
                </v-btn>
              </template>
            </v-card>
          </v-col>
        ...
        <v-row
          class="mt-1 justify-center px-2"
        ...
        <v-row>
          ...
          <v-col cols="12" class="mt-n3" v-if="category_type == 'fix'">
            <SelectSubCategory
          ...
          <v-row v-if="selectedAmountType == 'perpax'" class="px-3 mt-n3">
            <v-col cols="12" sm="5">
              <v-text-field
          ...
          <v-col
            v-if="
              (totalAmount != null || totalAmountPax > 0) && this.ready_edit > 1
            "
            cols="12"
            class="mt-n3 mb-3"
          >
          ...
          <v-col
            cols="12"
            :class="category_type == 'fix' ? 'mt-n3 py-0' : 'mt-n10 py-0'"
            v-if="
  ...
  >
    ...
    <template v-slot:actions>
      <v-btn color="white" variant="text" @click="snackbar = null">
        Close
      </v-btn>
    </template>
  </v-snackbar>
  ...
  <v-dialog v-model="approvalDialog" max-width="400">
    <v-card>
      ...
      <v-card-text> Are you sure you want to delete this draft? </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
...
export default {
  ...
  watch: {
    selectedCategory() {
      if (this.was_rendered) this.ready_edit++;
      if (this.ready_edit > 4) {
    ...
    selectedSubCategory() {
      ...
      if (this.ready_edit > 4) {
        ...
        this.manual_approval_btn = false;
      }
    },
    selectedCriteriaMap(newValue) {
      if (this.was_rendered) this.ready_edit++;
      if (this.ready_edit > 4 && newValue != 999) {
        this.amountPerPax = null;
        this.quantity = null;
        this.totalAmountPax = null;
        this.totalAmount = null;
  ...
  methods: {
    async fetchMemoDetails() {
      try {
        ...
        this.selectedCriteriaID = this.memo.crtr_id || null;
        ...
        if (myflowId) {
          try {
            const response_chat = await axios.patch(
              `/chat/notifications/${myflowId}`,
              { status: "create" }
            );
            if (draft_value) {
              this.snackbarMessage = "Draft saving successful";
